...
def write_r2_summary(outdir: str, rows: list[dict]) -> str:
    stamp = time.strftime("%H:%M:%S")
    root = os.path.join(outdir, "R2")
    os.makedirs(root, exist_ok=True)
    json_path = os.path.join(root, f"R2_summary_{stamp}.json")
    with open(json_path, "w", encoding="ascii") as f:
        f.write(__import__("json").dumps(rows, indent=2))
    csv_path = os.path.join(root, f"R2_summary_{stamp}.csv")
...
def plot_r2_sweep(outdir: str, rows: list[dict]) -> str | None:
    try:
    ...
    plt.title("R2 Latency Sensitivity Test")
    plt.legend()
    plt.grid(True, linestyle="--", alpha=0.4)
    out = os.path.join(root, "R2_latency_sweep.png")
    plt.tight_layout()
