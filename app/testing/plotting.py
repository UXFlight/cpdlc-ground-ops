...
def plot_basic(test_id: str, result: dict, params: dict, out_dir: str) -> list[str]:
    ...
    if end.get("p50") is not None and end.get("p95") is not None:
        ...
        out_png = os.path.join(out_dir, "latency_p50_p95.png")
        out_pdf = os.path.join(out_dir, "latency_p50_p95.pdf")
        plt.savefig(out_png, dpi=150, bbox_inches="tight")
        plt.savefig(out_pdf, bbox_inches="tight")
        outputs.extend([out_png, out_pdf])
        plt.close()
    if server.get("p50") is not None and server.get("p95") is not None:
        ...
        plt.title(_title(test_id, atc, pilots))
        out_png = os.path.join(out_dir, "server_processing_p50_p95.png")
        out_pdf = os.path.join(out_dir, "server_processing_p50_p95.pdf")
        plt.savefig(out_png, dpi=150, bbox_inches="tight")
        plt.savefig(out_pdf, bbox_inches="tight")
        outputs.extend([out_png, out_pdf])
        plt.close()
