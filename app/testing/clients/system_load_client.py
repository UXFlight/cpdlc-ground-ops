...
class SystemLoadClient:
    def __init__(self, 
        ...
        def _request_ack(data):
            if self.role == ROLE_PILOT and self._phase == PHASE_PENDING:
        ...
        def _atc_response(data):
            if self.role != ROLE_PILOT or self._phase not in {PHASE_PENDING, PHASE_REQUESTED}:
                return
        ...
        @self._sio.on("error")
        def _error(data):
            if self.role != ROLE_PILOT: return
